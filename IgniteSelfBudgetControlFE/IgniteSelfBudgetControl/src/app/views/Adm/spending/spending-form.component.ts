...
export class SpendingFormComponent implements OnInit {
  ...
  constructor(private _location: Location, private route: Router,private _service: SpendingControllerService,private accountService: AccountControllerService, private purposeService: PurposeControllerService, private creditStatusService: CreditStatusControllerService) { }

  ngOnInit() {
    this.accountId = history.state.accountId;
    if (this.accountId) {
      this.accountService.loadAccount(this.accountId).subscribe((res: any) => {
  ...
  cancel() {
    this.accountId ?  this._location.back() :  this.route.navigate(['/Adm/SpendingList'])
  }
